...
add_shortcode('vts_power_search', function ($atts) {
    ...
    <div class="vts-ps" data-vts-ps data-rest="<?= esc_attr(rest_url(VTS_NS)) ?>">
      <div class="vts-ps__head">
        ...
        <span class="vts-ps__badge"><?= esc_html(number_format_i18n($counts['engine'])) ?> wersji silnikowych</span>
      </div>

      <div class="vts-ps__row is-off">
        <span class="vts-ps__n">01</span>
        <div class="vts-ps__f">
          <span class="vts-ps__lab">Numer VIN</span>
          <div class="vts-ps__fake">Wpisz VIN — rozpoznamy auto automatycznie</div>
        </div>
        <span class="vts-ps__tag">wkrótce</span>
      </div>

      <div class="vts-ps__row">
        <span class="vts-ps__n">02</span>
        <div class="vts-ps__f">
          <span class="vts-ps__lab">Wybierz z listy</span>
          <div class="vts-ps__cascade">
            <select data-sel="make" aria-label="Marka">
              <option value="" selected disabled>Marka</option>
              <?php foreach ($makes as $m) : ?>
                <option value="<?= esc_attr($m['slug']) ?>"><?= esc_html($m['name']) ?></option>
              <?php endforeach; ?>
            </select>
            <select data-sel="model" aria-label="Model" disabled><option>Model</option></select>
            <select data-sel="gen" aria-label="Generacja" disabled><option>Generacja</option></select>
            <select data-sel="eng" aria-label="Silnik" disabled><option>Silnik</option></select>
          </div>
        </div>
      </div>

      <div class="vts-ps__row is-off">
        <span class="vts-ps__n">03</span>
        <div class="vts-ps__f">
          <span class="vts-ps__lab">Opisz własnymi słowami</span>
          <div class="vts-ps__fake">„mam golfa siódemkę 2.0 tdi” — asystent dobierze wersję</div>
        </div>
        <span class="vts-ps__tag">wkrótce</span>
      </div>
      ...
      <div class="vts-ps__out" data-out hidden>
        ...
        <form class="vts-ps__gate" data-gate novalidate>
          <p>Wynik dla <b data-f="veh">—</b> jest gotowy. Zostaw adres e-mail, a pokażemy
             pełne wartości po modyfikacji.</p>
